    ...
    public function build(Agency $agency, int $perPage = 20): array
    {
        ...
        $hostRoomAgg = LiveRoom::query()
            ...
            ->selectRaw("
                ...
                SUM(CASE WHEN room_type = 'audio' THEN GREATEST(TIMESTAMPDIFF(MINUTE, started_at, COALESCE(ended_at, last_activity_at, started_at)), 0) ELSE 0 END) as audio_room_minutes
            ")
            ->groupBy('host_id')
            ->get()
            ->keyBy('host_id');
