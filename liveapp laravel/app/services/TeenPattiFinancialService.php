...
class TeenPattiFinancialService
{
    public const GAME_KEY = 'teen_patti';

    public const COMMISSION_PERCENT = 5;

    public function recordBet(TeenPattiBet $bet): TeenPattiFinancialLedgerEntry
    {
    ...
    public function recordPayout(TeenPattiPayout $payout): TeenPattiFinancialLedgerEntry
    {
        ...
        return $this->applyEntry(
            eventKey: "teen_patti:payout:{$payout->id}:treasury_debit",
            eventType: 'payout_debit',
            treasuryDeltaCoins: -(int) $payout->payout_coins,
            commissionDeltaCoins: 0,
            roundId: (int) $payout->teen_patti_round_id,
            betId: (int) $payout->teen_patti_bet_id,
    ...
    public function recordRefund(TeenPattiBet $bet): TeenPattiFinancialLedgerEntry
    {
        $allocation = $this->recordBet($bet);

        return $this->applyEntry(
            eventKey: "teen_patti:bet:{$bet->id}:allocation_reversal",
    ...
    public function account(): TeenPattiFinancialAccount
    {
        return TeenPattiFinancialAccount::query()
    ...
    private function applyEntry(
        ...
        return DB::transaction(function () use (
            ...
            $existing = TeenPattiFinancialLedgerEntry::query()
            ...
            return TeenPattiFinancialLedgerEntry::query()->create([
                'teen_patti_financial_account_id' => $account->id,
                'teen_patti_round_id' => $roundId,
                'teen_patti_bet_id' => $betId,
                'teen_patti_payout_id' => $payoutId,
                'event_key' => $eventKey,
