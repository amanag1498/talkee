...
class AgencyWalletService
{
    ...
    public function transferToUser(
        ...
        if ($rechargePlanId <= 0) {
            throw new InvalidArgumentException('A recharge plan is required.');
        }
        ...
        return DB::transaction(function () use ($agency, $targetUser, $rechargePlanId, $admin, $agencyUser, $note, $reference) {
            $plan = RechargePlan::query()
                ->whereKey($rechargePlanId)
    ...
    public function paginatedTransfers(Agency $agency, int $perPage = 15, string $pageName = 'transfer_page'): LengthAwarePaginator
    {
        ...
        return AgencyCoinTransfer::query()
            ->with(['targetUser', 'admin', 'agencyUser', 'agencyWalletTransaction', 'userWalletTransaction', 'rechargePlan'])
            ->where('agency_wallet_id', $wallet->id)
            ->latest('id')
            ->paginate($perPage, ['*'], $pageName);
    }
    ...
    {
        return AgencyCoinTransfer::query()
            ->with(['agency.owner', 'targetUser', 'admin', 'agencyUser', 'agencyWalletTransaction', 'rechargePlan'])
