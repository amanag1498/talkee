  ...
  <div class="col-lg-7">
    <div class="card">
      <div class="card-header">
        <h6 class="mb-0"><i class="ti ti-edit me-2"></i>Edit Banner</h6>
      </div>
      <div class="card-body">
        ...
        <form method="post" action="{{ route('admin.banners.update', $banner) }}" enctype="multipart/form-data" class="vstack gap-3">
          @csrf @method('PUT')

          <div>
            <label class="form-label">Title</label>
            <input name="title" class="form-control @error('title') is-invalid @enderror" value="{{ old('title', $banner->title) }}" required maxlength="120">
          ...
          <div>
            ...
            <input name="image_url" class="form-control @error('image_url') is-invalid @enderror" value="{{ old('image_url', $banner->image_url) }}" maxlength="2048">
            @error('image_url') <div class="invalid-feedback">{{ $message }}</div> @enderror
          </div>

          <div>
            <label class="form-label">Target URL (optional)</label>
            <input name="target_url" class="form-control @error('target_url') is-invalid @enderror" value="{{ old('target_url', $banner->target_url) }}">
          ...
          <div class="row g-2">
            <div class="col-md-8">
              ...
              <input name="action_value" class="form-control @error('action_value') is-invalid @enderror" value="{{ old('action_value', $banner->action_value) }}" placeholder="URL / Deep link / Route">
              @error('action_value') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>
            <div class="col-md-4">
              <label class="form-label">Button Text</label>
              <input name="button_text" class="form-control @error('button_text') is-invalid @enderror" value="{{ old('button_text', $banner->button_text) }}" maxlength="60" placeholder="Open">
          ...
          <div class="row g-2">
            <div class="col-md-6">
              ...
              @foreach($platforms as $platform)
                <div class="form-check form-check-inline">
                  <input class="form-check-input" type="checkbox" id="platform_{{ $platform }}" name="platforms[]" value="{{ $platform }}" @checked(in_array($platform, $selectedPlatforms, true))>
            ...
            <div class="col-md-6">
              ...
              @foreach($roles as $role)
                <div class="form-check form-check-inline">
                  <input class="form-check-input" type="checkbox" id="role_{{ $role }}" name="target_roles[]" value="{{ $role }}" @checked(in_array($role, $selectedRoles, true))>
          ...
          <div class="row g-2">
            <div class="col-md-4">
              ...
              <input type="number" min="0" name="sort_order" class="form-control @error('sort_order') is-invalid @enderror" value="{{ old('sort_order', $banner->sort_order) }}">
          ...
          <div class="form-check">
            ...
            <input class="form-check-input" type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', (int) $banner->is_active) ? 'checked' : '' }}>
            <label for="is_active" class="form-check-label">Active</label>
          </div>

          <div class="d-flex gap-2">
            <button class="btn btn-primary"><i class="ti ti-check me-1"></i>Update</button>
            <a class="btn btn-light border" href="{{ route('admin.banners.index') }}">Back</a>
          </div>
        </form>
      </div>
    </div>
  ...
  <div class="col-lg-5">
    ...
    <div class="card">
      ...
      <div class="card-body text-center">
        @if($previewUrl !== '')
          <img src="{{ $previewUrl }}" alt="banner" style="max-height:140px; max-width:100%; object-fit:cover; border-radius:8px;" onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
          <small class="text-muted" style="display:none;">Image unavailable. Save a new image.</small>
        @else
          <small class="text-muted">No image on this banner yet.</small>
        @endif
      </div>
    </div>
  </div>
