...
public class RazorpayDelegate implements ActivityResultListener, ExternalWalletListener,
        PaymentResultWithDataListener, EventCallback {
    ...
    void subscribeToAnalyticsEvents(List<String> events, Result result) {
        try {
            ...
            if (events != null && !events.isEmpty()) {
                Method setSubscribed = Checkout.class.getDeclaredMethod(
                        "setSubscribedAnalyticsEvents", ArrayList.class);
    ...
    public void onPaymentError(int code, String message, PaymentData paymentData) {
        ...
        try {
            ...
            while (keys.hasNext()) {
                ...
                responseBody.put(key, errorObj.get(key));
            }
            responseBody.put("metadata", metadataHash);
            responseBody.put("email", paymentData != null ? paymentData.getUserEmail() : null);
            responseBody.put("contact", paymentData != null ? paymentData.getUserContact() : null);
            data.put("responseBody", responseBody);
        } catch (Exception e) {
            data.put("message", message);
            Map<String, Object> responseBody = new HashMap<>();
            responseBody.put("description", message);
            data.put("responseBody", responseBody);
        }
        reply.put("data", data);
        sendReply(reply);
    ...
    public void onPaymentSuccess(String paymentId, PaymentData paymentData) {
        ...
        if (paymentData.getData().has("razorpay_subscription_id")) {
            data.put(
                    ...
                    paymentData.getData().optString("razorpay_subscription_id"));
        }
        reply.put("data", data);
        sendReply(reply);
    }
    ...
    public boolean onActivityResult(int requestCode, int resultCode, Intent data) {
        try {
            Method merchantActivityResult = Checkout.class.getMethod(
                    ...
                    Integer.class,
                    Intent.class,
                    PaymentResultWithDataListener.class,
                    ExternalWalletListener.class);
            merchantActivityResult.invoke(
                    null, activity, requestCode, resultCode, data, this, this);
        } catch (Exception e) {
