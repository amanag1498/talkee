...
class SVGAParser {
  ...
  Future<ui.Image?> _decodeImageItem(String key, Uint8List bytes,
    ...
    try {
      ...
      try {
        final fallbackImage = await _decodeImageWithPureDart(bytes);
        if (fallbackImage != null) {
          if (task != null) {
            task.finish(
              arguments: {
                'imageSize': '${fallbackImage.width}x${fallbackImage.height}',
                'decoder': 'pure_dart_fallback',
              },
            );
          }
        ...
        if (task != null) {
          task.finish(
            arguments: {
              'error': '$fallbackError',
              'stack': '$fallbackStack',
              'decoder': 'pure_dart_fallback_failed',
            },
          );
        }
      }
  ...
  Future<ui.Image?> _decodeImageWithPureDart(Uint8List bytes) async {
